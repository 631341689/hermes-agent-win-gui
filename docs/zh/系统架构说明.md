...
| **知识库（Dashboard）** | 独立 REST：`/api/knowledge/*`；数据在 `HERMES_HOME/knowledge/`；分块（含 **`smart`**）、SSE 流式重建索引与协作取消；可选 MinerU PDF→Markdown；向量查询默认 **`knowledge.retrieval` 两阶段**（加宽 FAISS 召回 + 候选集 BM25/向量混合，可关） | 实现分散在 **`hermes_cli/knowledge_*.py`**（含 **`knowledge_rerank`**、`knowledge_index.query_vector_bases`）、**`tools/knowledge_tool.py`**；前端 **`web/src/pages/KnowledgePage.tsx`**；**`toolsets.py`** 工具集 **`knowledge`**。契约见 **`docs/zh/knowledge-api.md`** §1.4、§2.9、§4；可选 extras **`[knowledge]`** |
...
| **知识库 API** | `GET/POST /api/knowledge/…`；`POST .../reindex?stream=1`（SSE：`chunking` / `embedding` / `writing_index` / `final` / `error` / `cancelled`） | 鉴权与同 Dashboard；**`HERMES_ACTIVE_KB_IDS`**：供 Agent 工具在 **`kb_ids` 为空**时解析默认库（嵌入式 Chat 侧自动注入仍属横切规划，见知识库规划 §G）。详见 **`docs/zh/knowledge-api.md`**。 |
...
5. **知识库页**：对 **`/api/knowledge/*`** 的 CRUD、上传、**流式重建索引**（可终止）、调试 **`/query`**；与 **CLI/TUI 中的 Agent** 通过同一 **`HERMES_HOME/knowledge`** 注册表与 FAISS 索引衔接；模型侧检索经 **`tools/knowledge_tool.py`**（需启用 **`knowledge`** 工具集且具备 embedding 密钥）。
