...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "dashboard.locale": {
        "type": "select",
        "description": "Web dashboard UI language (auto follows browser; en/zh force English/Chinese)",
        "options": ["auto", "en", "zh"],
    },
    "dashboard.embedded_chat": {
        "type": "boolean",
        "description": "Enable Chat tab (embedded hermes --tui PTY). Restart dashboard after changing. Windows needs pywinpty (hermes-agent[web]).",
        "category": "display",
    },
...
def _runtime_pid_alive(pid: Any) -> bool:
    """True if ``pid`` looks like a live OS process (shared logic with ``get_running_pid``)."""
    try:
        n = int(pid)
    ...
    if n <= 0:
        return False
    return pid_alive_best_effort(n)
...
async def get_status():
    ...
    #
    # Read-only: never auto-delete ``gateway.pid`` / lock sidecars.  Polling
    # ``/api/status`` (5s) with ``cleanup_stale=True`` has caused Windows
    # false-negative lock/PID checks to unlink metadata while the messaging
    # gateway is still live — sidebar briefly wins then Feishu/socket churn.
    gateway_pid = get_running_pid(cleanup_stale=False)
    gateway_running = gateway_pid is not None
    remote_health_body: dict | None = None

    # Windows: ``get_running_pid`` can false-negative when mandatory byte-range
    # lock probes race the live gateway process. Same-host ``gateway.pid`` is
    # authoritative metadata — if it names a live ``hermes-gateway`` PID, trust
    # it for dashboard coexistence checks (no unlink; read-only).
    if not gateway_running:
        try:
            from gateway.status import _get_pid_path, _pid_from_record, _read_pid_record

            _rec = _read_pid_record(_get_pid_path())
            _fpid = _pid_from_record(_rec)
            if (
                _fpid is not None
                and _fpid != os.getpid()
                and isinstance(_rec, dict)
                and _rec.get("kind") == "hermes-gateway"
                and pid_alive_best_effort(_fpid)
            ):
    ...
    # ``gateway.pid`` / runtime-lock probing can miss a live gateway (profile
    # mismatch, Windows edge cases). Trust gateway_state.json when it claims
    # running and the recorded PID still exists.
    _runtime_early = read_runtime_status()
    if (
        not gateway_running
        and isinstance(_runtime_early, dict)
        and _runtime_early.get("gateway_state") == "running"
        and _runtime_pid_alive(_runtime_early.get("pid"))
    ):
        gateway_running = True
        try:
            gateway_pid = int(_runtime_early.get("pid"))
        except (TypeError, ValueError):
            pass
    ...
    if runtime:
        ...
        if not gateway_running:
            ...
            gateway_platforms = {}
        elif gateway_running and gateway_state in (None, "stopped"):
            # ``gateway_state.json`` can lag behind PID-file / process liveness
            # (restart handoff, Windows lock timing, or first write delayed).
            # Previously we only overrode when ``remote_health_body`` was set,
            # which left same-host + stale JSON showing「已停止」in the sidebar
            # while ``gateway_running`` was true.
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    try:
        ...
        _ppid = None
    _argv_repr = " ".join(sys.argv)
    log_file.write(
        (
            f"spawn_context: parent_pid={_ppid} this_pid={os.getpid()} "
            f"cmd={_argv_repr}\n"
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        # PYTHONIOENCODING: Windows dashboard spawns gateway restart/update as a child;
        # GBK consoles otherwise break on Unicode from Lark SDK / log formatters.
        "env": {
            **os.environ,
    ...
    if sys.platform == "win32":
        # DETACHED_PROCESS + inherited stdout (log file handle) can raise
        # WinError 87 on some Windows builds. CREATE_NO_WINDOW avoids a console
        # flash; CREATE_NEW_PROCESS_GROUP keeps signal semantics reasonable.
        win_flags = int(getattr(subprocess, "CREATE_NO_WINDOW", 0))
        win_flags |= int(getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0))
        if win_flags:
            popen_kwargs["creationflags"] = win_flags  # type: ignore[assignment]
    else:
...
async def restart_gateway():
    ...
    if _disable:
        _log.warning(
            "POST /api/gateway/restart rejected — "
            "HERMES_DASHBOARD_DISABLE_GATEWAY_RESTART is set"
        )
        raise HTTPException(
            status_code=403,
            detail=(
                "Gateway restart from the Dashboard is disabled "
                "(HERMES_DASHBOARD_DISABLE_GATEWAY_RESTART). "
                "Restart or stop `hermes gateway run` from the terminal "
                "where it is running."
            ),
        )
    _log.info(
        "POST /api/gateway/restart — will SIGTERM the messaging gateway for this "
        "HERMES_HOME if one is running (same as CLI: hermes gateway restart)."
    )
    try:
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                mc = get_model_capabilities(provider=provider, model=model_name)
                if mc is not None:
                    fam = getattr(mc, "model_family", "") or ""
...
def _resolve_chat_argv(
    ...
    # Ink spawns `python -m tui_gateway.entry`; without this, Windows often
    # resolves to the wrong interpreter (PATH/Conda) and the gateway exits
    # before chat works. Pin to the same interpreter as the dashboard.
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        dash_cfg = (load_config() or {}).get("dashboard") or {}
        raw_loc = str(dash_cfg.get("locale", "auto") or "auto").lower()
        locale_js = raw_loc if raw_loc in ("auto", "en", "zh") else "auto"
        token_script = (
            f'<script>window.__HERMES_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__HERMES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__HERMES_DASHBOARD_LOCALE__="{locale_js}";</script>'
        )
