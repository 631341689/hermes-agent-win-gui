...
DEFAULT_CONFIG = {
    ...
    "dashboard": {
        ...
        # Web UI language: auto (browser zh* → Chinese, else English), en, zh
        "locale": "auto",
        # In-browser PTY chat tab (same as `hermes dashboard --tui`). Needs a
        # POSIX PTY (macOS/Linux/WSL); native Windows shows a disabled banner.
        "embedded_chat": False,
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "HERMES_GATEWAY_DISABLE_STALE_CODE_CHECK": {
        "description": "When set to 1/true/yes/on, disables gateway stale-code auto-restart (Issue #17648) on all platforms. On Windows this check is already off unless HERMES_GATEWAY_ENABLE_STALE_CODE_CHECK is set",
        "prompt": "Disable stale-code gateway restart",
        "url": None,
        "password": False,
        "category": "setting",
    },
    "HERMES_GATEWAY_ENABLE_STALE_CODE_CHECK": {
        "description": "Windows only: set to 1/true/yes/on to enable gateway stale-code auto-restart (Issue #17648). Off by default on Windows so IDE/antivirus mtime noise does not SIGTERM the gateway; non-Windows platforms always run the check unless DISABLE is set",
        "prompt": "Enable stale-code gateway restart (Windows)",
        "url": None,
        "password": False,
        "category": "setting",
    },
    "HERMES_GATEWAY_WIN_USE_PROACTOR": {
        "description": "Windows only: use CPython's WindowsProactorEventLoopPolicy for the messaging gateway. Hermes defaults to WindowsSelectorEventLoopPolicy on Windows to avoid rare silent process exit with Feishu websockets plus background threads; set this when you need Proactor-specific behaviour",
        "prompt": "Gateway: use Windows Proactor asyncio policy",
        "url": None,
        "password": False,
        "category": "setting",
    },
    "HERMES_GATEWAY_FAULTHANDLER": {
        "description": "Gateway: enable Python faulthandler stack dumps on fault. On Windows this defaults ON (appends to HERMES_HOME/logs/gateway_faulthandler.log); set to 0/false/off to disable. On other platforms set to 1/true/on to enable",
        "prompt": "Gateway faulthandler (Windows default on)",
        "url": None,
        "password": False,
        "category": "setting",
    },
    "HERMES_GATEWAY_SKIP_KANBAN_EMBEDDED": {
        "description": "When 1/true/on, never start the embedded kanban notifier/dispatcher in the messaging gateway (use `hermes kanban daemon` instead). On Windows embedded kanban is already off unless HERMES_KANBAN_EMBED_IN_GATEWAY_WINDOWS=1; this env forces skip on all platforms. PowerShell: use `$env:HERMES_GATEWAY_SKIP_KANBAN_EMBEDDED='1'` (not cmd-style `set`)",
        "prompt": "Skip embedded kanban in gateway",
        "url": None,
        "password": False,
        "category": "setting",
    },
    "HERMES_KANBAN_EMBED_IN_GATEWAY_WINDOWS": {
        "description": "Windows only: when 1/true/on, start the embedded kanban notifier+dispatcher inside `hermes gateway run` (off by default so Feishu/Lark messaging stays isolated). The gateway adds extra startup delays so the first kanban ticks do not overlap the Feishu websocket handshake window",
        "prompt": "Embed kanban in gateway on Windows",
        "url": None,
        "password": False,
        "category": "setting",
    },
...
def _read_config_yaml_text(config_path: Path) -> str:
    """Decode ``config.yaml`` bytes for ``yaml.safe_load``.

    UTF-8 (with BOM) is tried first; GBK/cp936 cover Windows editors that
    save as \"ANSI\", which otherwise raises UnicodeDecodeError at startup.
    """
...
def read_raw_config() -> Dict[str, Any]:
    ...
    try:
        text = _read_config_yaml_text(config_path)
