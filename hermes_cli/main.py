...
# Windows: UTF-8 stdio before any logging or third-party prints (Feishu Lark SDK, etc.).
try:
...
def _launch_tui(
    ...
    if sys.platform == "win32":
        # Child `python -m tui_gateway` must emit UTF-8 JSON to a pipe; GBK
        # stdout raises UnicodeEncodeError on gateway.ready (skin glyphs).
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    if not npm:
        ...
        return not fatal
    # ASCII-only status lines — legacy Windows consoles (e.g. cp936/gbk)
    # cannot encode checkmark/cross glyphs used elsewhere in the CLI.
    print("-> Building web UI...")
    r1 = _run_npm_install_deterministic(npm, web_dir, extra_args=("--silent",))
    if r1.returncode != 0:
        tag = "ERROR" if fatal else "WARN"
        print(
            f"  [{tag}] Web UI npm install failed"
...
def _kill_stale_dashboard_processes(
    ...
    for pid, reason in failed:
        print(f"    [FAIL] failed to stop PID {pid}: {reason}")
...
def cmd_dashboard(args):
    ...
    embedded_from_cfg = False
    try:
        from hermes_cli.config import load_config as _load_dash_cfg

        _dc = _load_dash_cfg()
        _dash = _dc.get("dashboard") if isinstance(_dc.get("dashboard"), dict) else {}
        embedded_from_cfg = bool(_dash.get("embedded_chat", False))
    except Exception:
        embedded_from_cfg = False
    embedded_chat = (
        bool(getattr(args, "tui", False))
        or os.environ.get("HERMES_DASHBOARD_TUI") == "1"
        or embedded_from_cfg
    )
    start_server(
        host=args.host,
        port=args.port,
...
def main():
    ...
    gateway_run.add_argument(
        ...
        action="store_true",
        help=(
            "Windows only: show UAC and re-launch this gateway run as Administrator, "
            "then exit this process. Use when an elevated messaging gateway is required "
