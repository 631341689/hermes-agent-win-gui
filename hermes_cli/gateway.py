...
_logger_gw = logging.getLogger("hermes_cli.gateway")
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    if not all_profiles:
        try:
            ...
            # Read-only discovery — never unlink PID metadata while probing (Windows
            # polling + lock races caused intermittent gateway disconnects / sidebar).
            _append_unique_pid(pids, get_running_pid(cleanup_stale=False), _exclude)
        except Exception:
            pass
    ...
    *,
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            new_pid = get_running_pid(cleanup_stale=False)
        except Exception:
            new_pid = None
...
def stop_profile_gateway() -> bool:
    ...
    _logger_gw.warning(
        "Sending SIGTERM to messaging gateway pid=%s (invoked by hermes gateway "
        "stop/restart/replace, or dashboard POST /api/gateway/restart). "
        "Correlate timestamp with gateway.log 'Received SIGTERM'.",
        pid,
    )
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    remaining_pid = get_running_pid(cleanup_stale=False)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # Before any prints: Windows GBK consoles raise UnicodeEncodeError on box-drawing,
    # emoji, and on merged dashboard log streams if stdio is not UTF-8.
    try:
    ...
    # Pure ASCII only — must not use U+2500 box drawing, U+2695, or emoji.
    # A failed print here aborts the whole gateway / dashboard restart flow.
    _banner_lines = (
        "+---------------------------------------------------------+",
    ...
    try:
        ...
        print("Hermes Gateway Starting...  (Press Ctrl+C to stop)\n")
    
    # Exit with code 1 if gateway fails to connect any platform,
    # so systemd Restart=on-failure will retry on transient errors
    verbosity = None if quiet else verbose

    def _gateway_run_atexit() -> None:
        # Plain print — must survive even if logging handlers are already torn down.
        try:
            print(
                f"[hermes] gateway process atexit (pid={os.getpid()})",
                flush=True,
            )
        except Exception:
            pass

    atexit.register(_gateway_run_atexit)

    # Windows: faulthandler defaults on (see configure_gateway_faulthandler).
    configure_gateway_faulthandler()
    # Windows: Hermes defaults to Selector; Proactor remains opt-in via env.
    configure_windows_gateway_event_loop_policy()
    ...
    try:
        ...
        # asyncio.run propagates SystemExit from the coroutine (e.g. service restart code 75).
        try:
            print(f"[hermes] gateway exiting: SystemExit({exc.code})", flush=True)
        except Exception:
            pass
        raise
    if not success:
...
def _spawn_detached_gateway_run(*, replace: bool = False, verbose: int = 0) -> bool:
    """Start ``hermes gateway run`` in a detached subprocess; return False on failure.

    Windows manual ``gateway restart`` uses this so the restart CLI exits while
    the gateway keeps running (same model as Dashboard ``/api/gateway/restart``).
    """
    cmd = [get_python_path(), "-m", "hermes_cli.main", "gateway", "run"]
    if replace:
        cmd.append("--replace")
    if verbose > 0:
        cmd.extend(["-v"] * min(int(verbose), 3))
    try:
        popen_kwargs: dict = {
        ...
        if sys.platform == "win32":
            _wf = int(getattr(subprocess, "CREATE_NO_WINDOW", 0)) | int(
                getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0)
            )
            if _wf:
                popen_kwargs["creationflags"] = _wf
        else:
...
def gateway_setup():
    ...
    print(color("│             Hermes Gateway Setup                        │", Colors.MAGENTA))
    print(color("├─────────────────────────────────────────────────────────┤", Colors.MAGENTA))
...
def _windows_maybe_relaunch_gateway_elevated() -> bool:
    """If ``--elevate`` was used on Windows and we are not already Admin, UAC-relaunch.

    Returns True when an elevated child was spawned and this process should exit.
    """
    if sys.platform != "win32":
        return False
    try:
    ...
    argv = [arg for arg in sys.argv if arg != "--elevate"]
    if len(argv) < 1:
        return False
    exe = argv[0]
    tail = argv[1:]
    params = subprocess.list2cmdline(tail) if tail else ""
    try:
        cwd = str(Path.cwd())
    except Exception:
        cwd = ""
    ...
    try:
        ...
        # ShellExecuteW returns an HINSTANCE cast to size_t; values > 32 mean success.
        rc = int(
        ...
        print_error(f"Could not request Administrator elevation: {exc}")
        sys.exit(1)

    if rc <= 32:
        print_error(
            f"Elevation failed or was cancelled (ShellExecuteW returned {rc}). "
            "Try starting PowerShell with \"Run as administrator\" manually."
        )
        sys.exit(1)

    print_info(
        "Starting elevated gateway in a new window — this process exits so you "
        "can watch logs there."
    )
    return True
...
def _gateway_command_inner(args):
    ...
    if subcmd is None or subcmd == "run":
        if getattr(args, "elevate", False):
            if sys.platform != "win32":
                print_warning(
                    "--elevate is ignored on this platform (Windows-only)."
                )
            elif _windows_maybe_relaunch_gateway_elevated():
                sys.exit(0)
    ...
    if subcmd == "install":
        ...
        if restart_all:
            ...
            if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
                ...
                launchd_start()
            elif is_windows() and _spawn_detached_gateway_run(replace=False, verbose=0):
                _gl = get_hermes_home() / "logs" / "gateway.log"
                print(f"✓ Gateway process started (detached). Logs:\n  {_gl}")
            else:
        ...
        if not service_available:
            ...
            print("Starting gateway...")
            if is_windows():
                if _spawn_detached_gateway_run(replace=False, verbose=0):
                    _gl = get_hermes_home() / "logs" / "gateway.log"
                    print(f"✓ Gateway process started (detached). If it exits immediately, check:\n  {_gl}")
                    return
                print(
                    "⚠ Could not spawn detached gateway; running in foreground "
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("Windows: leave this terminal open while the gateway runs,")
                    print("  or start/restart from the Dashboard (Restart messaging gateway).")
                else:
                ...
                if is_termux():
                    ...
                    print("  Or use the Dashboard: Restart messaging gateway")
                else:
