...
# ``connect()`` must not run first-time schema init from two threads at once.
# The gateway starts the embedded kanban dispatcher and notifier with the same
# initial delay; without serialization, both can see ``needs_init`` and call
# ``executescript`` concurrently — undefined for SQLite and has correlated with
# abrupt whole-process exit on Windows (no Python traceback).
_path_connect_locks: dict[str, threading.Lock] = {}
_path_connect_locks_guard = threading.Lock()


def _connect_lock_for(resolved: str) -> threading.Lock:
    with _path_connect_locks_guard:
        lock = _path_connect_locks.get(resolved)
        if lock is None:
            lock = threading.Lock()
            _path_connect_locks[resolved] = lock
        return lock
...
def connect(db_path: Optional[Path] = None) -> sqlite3.Connection:
    ...
    lock = _connect_lock_for(resolved)
    with lock:
