...
class BaseEnvironment(ABC):
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        def _drain():
            # Windows: select() only supports sockets — using it on a subprocess
            # PIPE raises immediately, the drain thread exited with no reads, and
            # every terminal_tool call returned exit_code 0 with empty output (#Windows).
