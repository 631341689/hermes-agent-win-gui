...
def _win_gitbash_path_to_native(path: str) -> str:
    """Map Git Bash / MSYS ``pwd -P`` paths to a native path for Python/Popen cwd.

    Bash emits ``/d/repo`` but ``subprocess.Popen(cwd=...)`` on Windows requires
    ``D:\\repo`` — otherwise raises NotADirectoryError (#Windows terminal tool).
    """
    ...
    if len(p) >= 3 and p[0] == "/" and p[1].isalpha() and p[2] == "/":
        drive = p[1].upper()
        tail = p[3:].replace("/", os.sep)
        candidates.append(f"{drive}:{os.sep}{tail}" if tail else f"{drive}:{os.sep}")

    cyg = "/cygdrive/"
    low = p.lower()
    if low.startswith(cyg) and len(p) > len(cyg) + 2:
        drive = p[len(cyg)].upper()
        rest = p[len(cyg) + 2 :].lstrip("/")
        tail = rest.replace("/", os.sep) if rest else ""
        candidates.append(f"{drive}:{os.sep}{tail}" if tail else f"{drive}:{os.sep}")
...
def _make_run_env(env: dict) -> dict:
    ...
    # Windows PATH uses ';' and entries look like 'C:\...'.  Splitting on ':' breaks
    # drive letters and appending ':' + POSIX dirs corrupts the string — Git Bash
    # then loses System32 / Git / Python on PATH (#terminal failures on Windows).
    if not _IS_WINDOWS:
...
class LocalEnvironment(BaseEnvironment):
    ...
    def __init__(self, cwd: str = "", timeout: int = 60, env: dict = None):
        if cwd:
            ...
            cwd = _win_gitbash_path_to_native(cwd)
        super().__init__(cwd=cwd or os.getcwd(), timeout=timeout, env=env)
        self.cwd = _win_gitbash_path_to_native(self.cwd)
        self.init_session()
    ...
    def get_temp_dir(self) -> str:
        ...
        """
        # Windows: ``/tmp`` is wrong here — Python would open ``\\tmp\\`` under
        # the current drive and Git Bash snapshot paths must live under %TEMP%.
        # Forward slashes keep bash ``> path`` snippets free of ``\\t`` escapes.
        if _IS_WINDOWS:
            return os.path.normpath(tempfile.gettempdir()).replace("\\", "/")
    ...
    def _update_cwd(self, result: dict):
        ...
        try:
            ...
            if cwd_path:
                self.cwd = _win_gitbash_path_to_native(cwd_path)
