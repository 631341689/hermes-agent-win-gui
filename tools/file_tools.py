...
READ_FILE_SCHEMA = {
    "name": "read_file",
    "description": "Read a text file with line numbers and pagination. Use this instead of cat/head/tail in terminal. Output format: 'LINE_NUM|CONTENT'. Suggests similar filenames if not found. Use offset and limit for large files. Reads exceeding ~100K characters are rejected; use offset and limit to read specific sections of large files. NOTE: Cannot read images or binary files — use vision_analyze for images. On Windows, `/tmp/...` may refer to Git Bash's MSYS tree, not Windows %TEMP%.",
    "parameters": {
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Path to the file (absolute, relative, ~/path). Windows: prefer drive paths or TERMINAL_CWD-relative paths."},
...
WRITE_FILE_SCHEMA = {
    "name": "write_file",
    "description": "Write content to a file, completely replacing existing content. Use this instead of echo/cat heredoc in terminal. Creates parent directories automatically. OVERWRITES the entire file — use 'patch' for targeted edits. On Windows prefer workspace paths or %TEMP%-style directories (e.g. C:/Users/you/AppData/Local/Temp/...); `/tmp` is Unix-centric and under Git Bash maps to MSYS temp, not necessarily Windows %TEMP%.",
    "parameters": {
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Absolute or relative file path. Windows: use drive paths or paths under TERMINAL_CWD; avoid assuming `/tmp` unless you intend Git Bash's MSYS tmp."},
...
SEARCH_FILES_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "path": {"type": "string", "description": "Directory or file to search in (default: current working directory). On Windows avoid relying on `/tmp` unless you mean Git Bash's MSYS tmp — use workspace or %TEMP% paths for predictable results.", "default": "."},
