...
def get_model_capabilities(provider: str, model: str) -> Optional[ModelCapabilities]:
    ...
    # ``family`` must be a plain string for JSON + React; registry glitches
    # occasionally ship dict/list shapes that would crash UIs rendering it.
    _fam = entry.get("family", "")
    if isinstance(_fam, str):
        model_family = _fam.strip()
    elif isinstance(_fam, (int, float, bool)) or _fam is None:
        model_family = str(_fam) if _fam is not None else ""
    else:
        model_family = ""
