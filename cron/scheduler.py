...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Always bail out when nothing is due — even if verbose=False (gateway
        # ticker). Otherwise we still run load_config / orphan cleanup on every
        # 60s pulse for no reason.
        if not due_jobs:
            if verbose:
