...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const resumeFromUrl = useMemo(
    () => searchParams.get("resume")?.trim() || null,
    [searchParams],
  );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      {resumeFromUrl && (
        <div className="flex flex-col gap-2 border border-midground/25 bg-midground/5 px-3 py-2 text-xs tracking-wide text-midground/90 sm:flex-row sm:items-center sm:justify-between">
          <p className="min-w-0 flex-1 leading-snug">{t.sessions.chatResumeModelHint}</p>
          <Button
            type="button"
            ghost
            className="shrink-0 self-start border border-midground/40 sm:self-center"
            onClick={() => navigate("/chat")}
          >
            {t.sessions.chatStartFresh}
          </Button>
        </div>
      )}
