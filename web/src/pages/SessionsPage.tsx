...
export default function SessionsPage() {
  ...
  // Only show platform rows when the API says the gateway is up. Otherwise a
  // stale snapshot (e.g. last poll succeeded, next failed and kept old state)
  // reads as「已连接」while the sidebar shows「未运行」.
  const platformEntries =
    status && status.gateway_running
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {status?.gateway_running && platformEntries.length > 0 && (
