...
function hermesDevToken(): Plugin {
  ...
  const LOCALE_RE =
    /window\.__HERMES_DASHBOARD_LOCALE__\s*=\s*"(auto|en|zh)"/;
      ...
      try {
        ...
        const embeddedJs = embeddedMatch
          ...
          : legacyMatch
            ? legacyMatch[1]
            : "false";
        const localeMatch = html.match(LOCALE_RE);
        const localeJs = localeMatch?.[1] ?? "auto";
        return [
          {
            tag: "script",
            injectTo: "head",
            children:
              ...
              `window.__HERMES_DASHBOARD_LOCALE__="${localeJs}";`,
          },
        ];
      } catch (err) {
        console.warn(
