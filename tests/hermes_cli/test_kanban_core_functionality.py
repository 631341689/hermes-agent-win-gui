...
def test_connect_concurrent_first_open_is_safe(tmp_path, monkeypatch):
    """Embedded gateway starts kanban dispatcher + notifier together; both
    must be able to call connect() on a cold DB without racing two inits."""
    ...
    def _open() -> None:
        c = kb.connect()
        try:
            c.execute("SELECT 1 FROM tasks LIMIT 1").fetchone()
        finally:
            c.close()

    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
        list(pool.map(lambda _: _open(), range(16)))
