...
@pytest.mark.skipif(sys.platform == "win32", reason="systemd / os.getuid not available on Windows")
...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX geteuid / systemd setup")
...
def test_find_gateway_pids_falls_back_to_pid_file_when_process_scan_fails(monkeypatch):
    ...
    monkeypatch.setattr("gateway.status.get_running_pid", lambda *a, **k: 321)
...
class TestWaitForGatewayExit:
    ...
    def test_handles_process_already_gone_on_kill(self, monkeypatch):
        ...
        monkeypatch.setattr("time.monotonic", fake_monotonic)
        monkeypatch.setattr("time.sleep", lambda _: None)
        monkeypatch.setattr("gateway.status.get_running_pid", lambda *a, **k: 99)
        monkeypatch.setattr(gateway, "terminate_pid", mock_terminate)
...
class TestStopProfileGateway:
    def test_stop_profile_gateway_keeps_pid_file_when_process_still_running(self, monkeypatch):
        ...
        monkeypatch.setattr("gateway.status.get_running_pid", lambda *a, **k: 12345)
        monkeypatch.setattr(
            gateway.os,
            "kill",
