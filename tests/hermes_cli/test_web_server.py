...
class TestWebServerEndpoints:
    ...
    def test_get_status_get_running_pid_is_non_destructive(self, monkeypatch):
        """Dashboard polls often; never unlink PID files from status probes (#17648-style)."""
        ...
        monkeypatch.setattr(web_server, "get_running_pid", _probe)
        self.client.get("/api/status")
        assert len(calls) == 1
        assert calls[0]["kwargs"].get("cleanup_stale") is False
    ...
    def test_get_status_filters_unconfigured_gateway_platforms(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "get_running_pid", lambda *a, **k: 1234)
        monkeypatch.setattr(
            web_server,
            "read_runtime_status",
            lambda: {
    ...
    def test_get_status_running_overrides_stale_stopped_in_runtime_json(
        self, monkeypatch
    ):
        """Same-host: PID says gateway is up but gateway_state.json still says stopped (restart lag)."""
        import gateway.config as gateway_config
        import hermes_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "get_running_pid", lambda *a, **k: 4242)
        monkeypatch.setattr(
            web_server,
            "read_runtime_status",
            lambda: {
        ...
        assert "feishu" in data["gateway_platforms"]

    def test_get_status_pid_file_fallback_when_get_running_pid_false(
        self, monkeypatch, tmp_path
    ):
        """Windows: lock probe can miss a live gateway; trust gateway.pid + liveness."""
        import gateway.config as gateway_config
        ...
        monkeypatch.setattr(web_server, "pid_alive_best_effort", lambda pid: pid == 7777)
        monkeypatch.setattr(
            gs,
            "_read_pid_record",
            lambda *_a, **_k: {
        ...
        monkeypatch.setattr(gs, "_get_pid_path", lambda: tmp_path / "gateway.pid")
        monkeypatch.setattr(web_server, "check_config_version", lambda: (1, 1))
        monkeypatch.setattr(gateway_config, "load_gateway_config", lambda: _GatewayConfig())
...
class TestStatusRemoteGateway:
    ...
    def test_status_falls_back_to_remote_probe(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "get_running_pid", lambda *a, **k: None)
    ...
    def test_status_remote_probe_not_attempted_when_local_pid_found(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "get_running_pid", lambda *a, **k: 1234)
...
class TestResolveChatArgvEnv:
    """_resolve_chat_argv env for embedded Chat PTY."""

    def test_pins_hermes_python_to_sys_executable(self, monkeypatch):
        import hermes_cli.web_server as ws

        monkeypatch.delenv("HERMES_PYTHON", raising=False)

        def fake_make_tui_argv(tui_dir, tui_dev=False):
            return (["true"], str(tui_dir))

        monkeypatch.setattr("hermes_cli.main._make_tui_argv", fake_make_tui_argv)
        _argv, _cwd, env = ws._resolve_chat_argv()
        assert env.get("HERMES_PYTHON") == sys.executable

    def test_preserves_explicit_hermes_python(self, monkeypatch):
        import hermes_cli.web_server as ws
        ...
        monkeypatch.setattr("hermes_cli.main._make_tui_argv", fake_make_tui_argv)
        monkeypatch.setenv("HERMES_PYTHON", r"C:\special\python.exe")
        _argv, _cwd, env = ws._resolve_chat_argv()
        assert env["HERMES_PYTHON"] == r"C:\special\python.exe"
