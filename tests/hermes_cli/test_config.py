...
class TestReadConfigYamlEncoding:
    """GBK/ANSI-saved config.yaml must load on Windows (Issue: utf-8 decode errors)."""

    def test_read_accepts_gbk_bytes(self, tmp_path):
        path = tmp_path / "cfg.yaml"
        path.write_bytes("model:\n  provider: deepseek\n# \u6d4b\n".encode("gbk"))
        text = _read_config_yaml_text(path)
        assert "deepseek" in text
        assert "\u6d4b" in text

    def test_load_config_merges_gbk_yaml_file(self, tmp_path):
        config_path = tmp_path / "config.yaml"
        # Non-ASCII in GBK that is not valid UTF-8 — old code failed here.
