...
def test_detect_stale_code_true_after_update(tmp_path, monkeypatch):
    """Sentinel files newer than boot snapshot → stale."""
    # Windows defaults stale-code check off; tests assume Linux-style opt-out.
    if sys.platform == "win32":
        monkeypatch.setenv("HERMES_GATEWAY_ENABLE_STALE_CODE_CHECK", "1")
    repo = _make_tmp_repo(tmp_path)
    baseline = time.time() - 100
    for rel in _STALE_CODE_SENTINELS:
        os.utime(repo / rel, (baseline, baseline))
...
def test_detect_stale_code_ignores_subsecond_drift(tmp_path, monkeypatch):
...
def test_detect_stale_code_win32_skipped_without_opt_in(tmp_path, monkeypatch):
    """On Windows, newer sentinels do not trigger stale unless ENABLE is set."""
    monkeypatch.setattr(sys, "platform", "win32", raising=False)
    monkeypatch.delenv("HERMES_GATEWAY_ENABLE_STALE_CODE_CHECK", raising=False)
    monkeypatch.delenv("HERMES_GATEWAY_DISABLE_STALE_CODE_CHECK", raising=False)
    repo = _make_tmp_repo(tmp_path)
    baseline = time.time() - 100
    for rel in _STALE_CODE_SENTINELS:
        os.utime(repo / rel, (baseline, baseline))
...
def test_detect_stale_code_win32_opt_in(tmp_path, monkeypatch):
    monkeypatch.setattr(sys, "platform", "win32", raising=False)
    monkeypatch.setenv("HERMES_GATEWAY_ENABLE_STALE_CODE_CHECK", "1")
    repo = _make_tmp_repo(tmp_path)
    baseline = time.time() - 100
