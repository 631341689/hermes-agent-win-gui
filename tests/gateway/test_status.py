...
class TestGatewayPidState:
    def test_pid_alive_via_kill0_falls_back_to_tasklist_on_windows(
        self, monkeypatch
    ):
        """When os.kill fails spuriously, Windows uses tasklist (mocked here)."""
        monkeypatch.setattr(status, "_IS_WINDOWS", True)

        def boom_kill(_p, _s):
            raise OSError(87, "simulated")

        monkeypatch.setattr(status.os, "kill", boom_kill)
        ...
        def fake_run(**kwargs):
            ...
            return subprocess.CompletedProcess(
                kwargs["args"],
                0,
                stdout="python.exe                   4242 Console                1    123,456 K\n",
            )

        monkeypatch.setattr(status.subprocess, "run", fake_run)
        assert status._pid_alive_via_kill0(4242) is True
        assert called["n"] >= 1
    ...
    def test_unlink_stale_gateway_pid_after_lock_allows_excl_write(
        self, tmp_path, monkeypatch
    ):
        """Non-destructive get_running_pid can leave gateway.pid; post-lock cleanup must not block startup."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        pid_path = tmp_path / "gateway.pid"
        dead_pid = 999999
        pid_path.write_text(json.dumps({
        ...
        assert status.get_running_pid(cleanup_stale=False) is None
        assert pid_path.exists()

        assert status.acquire_gateway_runtime_lock() is True
        try:
            status.unlink_stale_gateway_pid_after_runtime_lock()
            assert not pid_path.exists()
            status.write_pid_file()
    ...
    def test_get_running_pid_on_windows_uses_pid_file_not_proc(
        self, tmp_path, monkeypatch
    ):
        """Regression: /api/status must not touch /proc on Windows (WinError 87)."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_IS_WINDOWS", True)
        pid_path = tmp_path / "gateway.pid"
        pid_path.write_text(json.dumps({
        ...
        def boom_proc(pid):
            proc_called["n"] += 1
            raise OSError(87, "Simulated WinError on /proc access")

        monkeypatch.setattr(status, "_read_process_cmdline", boom_proc)
        ...
        try:
            ...
            status.release_gateway_runtime_lock()
        assert proc_called["n"] == 0

    def test_get_running_pid_windows_fallback_when_lock_probe_false_but_foreign_gateway_alive(
        self, tmp_path, monkeypatch
    ):
        """Lock probe can false-negative on Windows; do not wipe a valid gateway.pid."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_IS_WINDOWS", True)
        monkeypatch.setattr(status, "is_gateway_runtime_lock_active", lambda lock_path=None: False)
            ...
            json.dumps(
                {
                    "pid": foreign_pid,
                    "kind": "hermes-gateway",
                    "argv": ["python", "hermes_cli/main.py", "gateway", "run"],
                    "start_time": None,
                }
            )
        )
        ...
        assert status.get_running_pid() == foreign_pid
        assert pid_path.exists()

    def test_get_running_pid_treats_systemerror_from_kill0_as_dead(
        self, tmp_path, monkeypatch
    ):
        """Regression: on Windows, invalid PID can raise SystemError (WinError 87), not OSError."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        pid_path = tmp_path / "gateway.pid"
        stale_pid = 999999
        pid_path.write_text(
        ...
        def kill_simulated(p, s):
            if p == stale_pid and s == 0:
                raise SystemError("built-in function kill returned a result with an exception set")
            return None

        monkeypatch.setattr(status.os, "kill", kill_simulated)
    ...
    def test_get_running_pid_on_windows_accepts_kind_without_argv_pattern(
        self, tmp_path, monkeypatch
    ):
        """Windows: no /proc; kind=hermes-gateway must win even if argv is non-standard."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_IS_WINDOWS", True)
        pid_path = tmp_path / "gateway.pid"
        pid_path.write_text(
            json.dumps(
                {
                    "pid": os.getpid(),
                    "kind": "hermes-gateway",
                    "argv": [r"C:\App\python.exe", "-c", "import runpy"],
                    "start_time": None,
                }
            )
        )
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_refreshes_argv_from_current_process(
        self, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        state_path = tmp_path / "gateway_state.json"
    ...
    def test_reconcile_stale_gateway_runtime_status_unlinks_dead_pid(
        self, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        dead = 999996
        (tmp_path / "gateway_state.json").write_text(
            json.dumps({"pid": dead, "gateway_state": "starting", "argv": []})
        )

        def _alive(pid: int) -> bool:
            return pid == os.getpid()

        monkeypatch.setattr(status, "pid_alive_best_effort", _alive)

        status.reconcile_stale_gateway_runtime_status()

        assert not (tmp_path / "gateway_state.json").exists()

    def test_reconcile_stale_gateway_runtime_status_keeps_live_pid(
        self, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        alive_pid = 888887
        (tmp_path / "gateway_state.json").write_text(
            json.dumps({"pid": alive_pid, "gateway_state": "running"})
        )

        monkeypatch.setattr(
            status, "pid_alive_best_effort", lambda p: p == alive_pid
        )
...
def test_process_helpers_skip_procfs_when_windows_flag(monkeypatch):
    """Regression: /proc paths are invalid on Windows — avoid Path + read."""
    monkeypatch.setattr(status, "_IS_WINDOWS", True)
    assert status._get_process_start_time(os.getpid()) is None
    assert status._read_process_cmdline(os.getpid()) is None
