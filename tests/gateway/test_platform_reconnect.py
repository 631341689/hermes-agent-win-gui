...
class TestRuntimeDisconnectQueuing:
    ...
    @pytest.mark.asyncio
    async def test_retryable_error_stays_alive_when_all_down_and_queued(self):
        """When the last adapter dies with a retryable error, stay running so
        `_platform_reconnect_watcher` can reconnect (interactive / Windows)."""
        runner = _make_runner()
        runner.stop = AsyncMock()
