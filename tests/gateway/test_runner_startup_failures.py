...
@pytest.mark.asyncio
async def test_runner_startup_failure_includes_adapter_none_reason(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(runner, "_create_adapter", lambda platform, platform_config: None)

    ok = await runner.start()

    assert ok is False
    assert runner._startup_failure_reason is not None
    assert "telegram" in runner._startup_failure_reason
    assert "adapter not created" in runner._startup_failure_reason
    state = read_runtime_status()
    assert state["gateway_state"] == "startup_failed"
    assert "adapter not created" in (state.get("exit_reason") or "")
...
async def test_start_gateway_replace_clears_marker_on_permission_denied(
    ...
    monkeypatch.setattr("gateway.status.get_running_pid", lambda *a, **k: 42)
