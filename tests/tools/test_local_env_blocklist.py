...
class TestWinGitbashPathNative:
    """Git Bash cwd strings must map to native paths for subprocess.Popen cwd."""

    def test_msys_drive_prefix(self, monkeypatch):
        import tools.environments.local as loc

        monkeypatch.setattr(loc, "_IS_WINDOWS", True)
        monkeypatch.setattr(os.path, "isdir", lambda p: True)
        out = loc._win_gitbash_path_to_native("/d/hermes-agent/repo")
        assert "D:" in out
        assert "hermes-agent" in out.replace("\\", "/")

    def test_unknown_path_unchanged_when_not_dir(self, monkeypatch):
        import tools.environments.local as loc

        monkeypatch.setattr(loc, "_IS_WINDOWS", True)
        monkeypatch.setattr(os.path, "isdir", lambda p: False)
        raw = "/z/definitely/missing"
        assert loc._win_gitbash_path_to_native(raw) == raw
...
class TestSanePathIncludesHomebrew:
    ...
    def test_make_run_env_does_not_duplicate_on_full_path(self, monkeypatch):
    ...
    def test_make_run_env_preserves_windows_path(self, monkeypatch):
        """Regression: never append POSIX _SANE_PATH or split PATH on ':' on Windows."""
        from tools.environments import local

        monkeypatch.setattr(local, "_IS_WINDOWS", True)
        win_path = r"C:\Windows\system32;C:\Windows;D:\Git\Git\bin"
        with patch.dict(os.environ, {"PATH": win_path}, clear=True):
            result = local._make_run_env({})
        assert result["PATH"] == win_path
        assert "/opt/homebrew" not in result["PATH"]
