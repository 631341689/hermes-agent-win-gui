...
class TestWaitForProcessWindowsPipeDrain:
    """Regression: Windows cannot select() on PIPE fds — use blocking reads."""

    def test_collects_stdout_when_using_win32_drain_branch(self, monkeypatch, tmp_path):
        monkeypatch.setattr(sys, "platform", "win32")

        class _Env(BaseEnvironment):
            def _run_bash(self, cmd_string, *, login=False, timeout=120, stdin_data=None):
                # Use the test interpreter — avoids broken WSL bash entries on Windows
                return subprocess.Popen(
                    [sys.executable, "-c", "print('win32_drain_ok')"],
        ...
        env = _Env(cwd=str(tmp_path), timeout=30)
        proc = env._run_bash("")
        result = env._wait_for_process(proc, timeout=15)
        assert "win32_drain_ok" in result["output"]
