...
const resolvePython = (root: string) => {
  ...
  // Windows repo venvs live under Scripts/python.exe — without these,
  // `npm run dev` without an activated venv falls through to `python`
  // on PATH (often Conda) and the gateway subprocess exits immediately.
  const hit = [
...
export class GatewayClient extends EventEmitter {
  ...
  start() {
    ...
    // Windows GBK stdout breaks gateway.ready JSON (skin emoji); force UTF-8
    // for the gateway subprocess unless the user already set these.
