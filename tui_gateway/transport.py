...
class StdioTransport:
    ...
    On Windows the JSON-RPC pipe often inherits a GBK-encoded text ``stdout``.
    Writing ``gateway.ready`` (skin strings with emoji / symbols) then raises
    ``UnicodeEncodeError`` and the TUI shows ``gateway exited``.  When the
    stream exposes a UTF-8 ``.buffer`` (``TextIOWrapper``), write UTF-8 bytes
    there instead of Unicode through the text codec.
    """
    ...
    def _write_line(stream: Any, line: str) -> None:
        ...
        if buf is not None and not isinstance(stream, io.StringIO):
