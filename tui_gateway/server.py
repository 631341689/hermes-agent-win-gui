...
def _probe_config_health(cfg: dict) -> str:
    ...
    if env_model:
        ...
        if isinstance(mc, dict):
            yaml_default = str(mc.get("default") or mc.get("model") or "").strip()
        if yaml_default and env_model != yaml_default:
            warnings.append(
                f"HERMES_MODEL / HERMES_INFERENCE_MODEL is `{env_model}` and overrides "
                f"config default `{yaml_default}`. Unset those env vars to use Models page defaults."
            )
