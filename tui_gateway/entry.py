...
def _log_signal(signum: int, frame) -> None:
    ...
    """
    _sig_names = {signal.SIGTERM: "SIGTERM"}
    if hasattr(signal, "SIGPIPE"):
        _sig_names[signal.SIGPIPE] = "SIGPIPE"  # type: ignore[attr-defined]
    if hasattr(signal, "SIGHUP"):
        _sig_names[signal.SIGHUP] = "SIGHUP"  # type: ignore[attr-defined]
    name = _sig_names.get(signum, f"signal {signum}")
    try:
...
# Windows has no SIGPIPE/SIGHUP — registering them raises AttributeError
# and kills the gateway before gateway.ready (embedded chat shows
# "gateway exited" with queued messages).
if hasattr(signal, "SIGPIPE"):
    signal.signal(signal.SIGPIPE, signal.SIG_IGN)  # type: ignore[attr-defined]
signal.signal(signal.SIGTERM, _log_signal)
if hasattr(signal, "SIGHUP"):
    signal.signal(signal.SIGHUP, _log_signal)  # type: ignore[attr-defined]
