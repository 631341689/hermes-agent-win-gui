...
# ``lark_oapi.ws.client`` binds ``loop = asyncio.get_event_loop()`` at import time.
# Importing it while ``gateway.platforms.feishu`` loads (gateway main loop already
# running) pins that module global to the gateway loop and correlates with rare
# Windows exits before ``FeishuAdapter`` construction finishes.  See
# ``_get_feishu_ws_client_cls`` — import is deferred to the websocket worker.
_feishu_ws_client_cls: Any = None


def _get_feishu_ws_client_cls() -> Any:
    global _feishu_ws_client_cls
    if _feishu_ws_client_cls is None:
        from lark_oapi.ws import Client as _Cls  # noqa: PLC0415

        _feishu_ws_client_cls = _Cls
    return _feishu_ws_client_cls
...
def _parse_feishu_card_action_value(value: Any) -> Optional[Dict[str, Any]]:
    """Normalize Feishu ``card.action.trigger`` action value to a dict.

    Newer clients / JSON 2.0 paths may deliver ``value`` as a JSON string; the
    Lark SDK model types it as dict only.
    """
    if value is None:
        return None
...
def _coerce_feishu_approval_id(raw: Any) -> Any:
    """Align callback ``approval_id`` with ``_approval_state`` keys (ints)."""
    if raw is None:
        return None
...
def _lark_expiring_cache_init_no_bg_cron(self: Any, clear_interval: int = 60) -> None:
    """Replacement for ``ExpiringCache.__init__`` while the Lark WS client is built.

    The stock implementation does ``asyncio.get_event_loop().create_task(...)``.
    On Windows, ``get_event_loop()`` in a worker thread can still resolve to the
    **gateway's** main event loop; ``create_task`` from the wrong thread then
    corrupts asyncio and the whole CPython process can exit with no traceback
    (often right after the first websocket frames arrive).

    The WS merge-buffer cache still works: ``get()`` evicts expired entries;
    without the cron task only periodic bulk ``_clear`` is skipped.
    """
    self._cache = {}
...
def _lark_expiring_cache_del_safe(self: Any) -> None:
...
def _run_official_feishu_ws_client(adapter: Any, ws_client_factory: Callable[[], Any]) -> None:
    """Run the official Lark WS client in its own thread-local event loop.

    ``ws_client_factory`` must be a zero-arg callable that returns a fresh
    ``lark_oapi.ws.Client``. The client **must** be constructed only after
    this thread calls ``asyncio.set_event_loop`` on the new loop: Lark's
    ``Client.__init__`` creates ``asyncio.Lock()`` which binds to the *current*
    running loop at construction time. Building the client on the gateway's
    main loop and then calling ``start()`` (which uses ``run_until_complete``)
    on a different thread loop leaves the lock on the wrong loop — undefined
    behaviour on POSIX and observed silent process exit shortly after the
    websocket handshake on Windows.
    """
    import lark_oapi.ws.client as ws_client_module

    # lark_oapi binds ``ws.client.loop`` at import time (often the gateway's
    # asyncio loop). We replace it with this thread's loop while ``start()``
    # runs, then **restore** the previous binding. Leaving a reference to a
    # closed thread-local loop in the module has caused the main Proactor loop
    # on Windows to misbehave or exit shortly after startup (no traceback on
    # the console — only an abrupt return to the shell).
    previous_module_loop = getattr(ws_client_module, "loop", None)
    ...
    _ec_mod: Any = None
    _saved_ec_init: Any = None
    _saved_ec_del: Any = None
    _ec_patch_applied = False
    try:
        import lark_oapi.core.cache.expiring_cache as _ec_mod

        _saved_ec_init = _ec_mod.ExpiringCache.__init__
        _saved_ec_del = getattr(_ec_mod.ExpiringCache, "__del__", None)
        _ec_mod.ExpiringCache.__init__ = _lark_expiring_cache_init_no_bg_cron
        if _saved_ec_del is not None:
            _ec_mod.ExpiringCache.__del__ = _lark_expiring_cache_del_safe
        _ec_patch_applied = True
    except Exception:
        logger.debug(
            "[Feishu] Could not patch Lark ExpiringCache (continuing without patch)",
            exc_info=True,
        )

    ws_client: Any = None
    original_connect: Any = None
    original_configure: Any = None
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "[Feishu] Websocket client start() raised — "
                "connection thread ending (check network / Feishu outage / duplicate gateway)",
            )
        else:
            # Long-lived clients normally block until disconnect; a clean return means
            # the SDK stopped its loop (often followed by silent loss of inbound events).
            logger.warning(
                "[Feishu] Websocket client start() returned — connection thread ending",
            )
    finally:
        if _ec_patch_applied and _ec_mod is not None and _saved_ec_init is not None:
            try:
                _ec_mod.ExpiringCache.__init__ = _saved_ec_init
                if _saved_ec_del is not None:
                    _ec_mod.ExpiringCache.__del__ = _saved_ec_del
            except Exception:
                logger.debug("[Feishu] ExpiringCache class restore failed", exc_info=True)
        if original_connect is not None:
            ws_client_module.websockets.connect = original_connect
        if ws_client is not None and original_configure is not None:
        ...
        try:
            ws_client_module.loop = previous_module_loop
        except Exception:
            pass
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Lark ``start()`` blocks for the socket lifetime. Use a plain daemon
        # ``threading.Thread`` plus a main-loop ``Future`` — not
        # ``run_in_executor`` / ``ThreadPoolExecutor``. Mixing asyncio executors
        # with this nested asyncio loop on Windows correlated with abrupt process
        # exit after WSS connect (no traceback).
        self._ws_feishu_thread: Optional[threading.Thread] = None
    ...
    async def disconnect(self) -> None:
        ...
        ws_thr = self._ws_feishu_thread
        self._ws_feishu_thread = None
        if ws_thr is not None and ws_thr.is_alive():
            try:
                ws_thr.join(timeout=2.0)
            except Exception:
                pass
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            def _btn_v2(element_id: str, label: str, action_name: str, btn_type: str = "default") -> dict:
                # JSON 2.0: interactive buttons use ``behaviors`` + ``callback`` (not 1.0 ``action`` blocks).
                return {
                    "tag": "button",
            ...
            card = {
                ...
                "body": {
                    ...
                    "elements": [
                        {
                            "tag": "markdown",
                            "element_id": "approval_cmd",
                            "content": f"```\n{cmd_preview}\n```\n**Reason:** {description}",
                        },
                        {
                            "tag": "column_set",
                            "element_id": "approval_btns",
                            "flex_mode": "flow",
                            ...
                            "columns": [
                                {
                                    ...
                                    "weight": 1,
                                    "vertical_align": "top",
                                    "elements": [_btn_v2("btn_once", "✅ Allow Once", "approve_once", "primary")],
                                },
                                {
                                    "tag": "column",
                                    "width": "weighted",
                                    "weight": 1,
                                    "vertical_align": "top",
                                    "elements": [_btn_v2("btn_sess", "✅ Session", "approve_session")],
                                },
                                {
                                    "tag": "column",
                                    "width": "weighted",
                                    "weight": 1,
                                    "vertical_align": "top",
                                    "elements": [_btn_v2("btn_alws", "✅ Always", "approve_always")],
                                },
                                {
                                    "tag": "column",
                                    "width": "weighted",
                                    "weight": 1,
                                    "vertical_align": "top",
                                    "elements": [_btn_v2("btn_deny", "❌ Deny", "deny", "danger")],
                                },
                            ],
                        },
                    ],
    ...
    def _build_resolved_approval_card(*, choice: str, user_name: str) -> Dict[str, Any]:
        """Build raw card JSON (JSON 2.0) for ``card.action.trigger`` inline update."""
    ...
    def _on_message_event(self, data: Any) -> None:
        ...
        if not self._loop_accepts_callbacks(loop):
            ...
            return
        if self._submit_coro_on_gateway_loop(self._handle_message_event_data(data)):
            return
        # Loop raced closed between _loop_accepts_callbacks and call_soon — replay.
        start_drainer = self._enqueue_pending_inbound_event(data)
        if start_drainer:
            threading.Thread(
                target=self._drain_pending_inbound_events,
        ...
        try:
            while True:
                ...
                if self._loop_accepts_callbacks(loop):
                    ...
                    requeue: List[Any] = []
                    for event in batch:
                        coro = self._handle_message_event_data(event)
                        if self._submit_coro_on_gateway_loop(coro):
                            dispatched += 1
                        else:
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        ...
        logging, and reaction. Scheduling uses ``_submit_coro_on_gateway_loop``
        (same as ``_on_message_event``).
        """
        ...
        if not self._loop_accepts_callbacks(loop):
            ...
            return
        self._submit_coro_on_gateway_loop(
    ...
    def _on_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        if (
            ...
            return
        self._submit_coro_on_gateway_loop(self._handle_reaction_event(event_type, data))
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        action_value = _parse_feishu_card_action_value(getattr(action, "value", None)) or {}
    ...
    def _submit_coro_on_gateway_loop(self, coro: Any) -> bool:
        """Hand *coro* to the gateway loop from the Lark SDK / helper threads.

        Uses ``call_soon_threadsafe`` + ``asyncio.create_task`` instead of
        :func:`asyncio.run_coroutine_threadsafe`. The latter funnels through
        ``concurrent.futures``; on some Windows + SelectorEventLoop + Feishu
        websocket stacks the first post-handshake frames have been observed to
        terminate the whole CPython process with no Python traceback and no
        gateway shutdown.

        Returns:
            True if submission was queued, False if the coroutine was not
            scheduled (caller may requeue the triggering event).
        """
        loop = self._loop
        if not self._loop_accepts_callbacks(loop):
            try:
    ...
    def _submit_on_loop(self, loop: Any, coro: Any) -> None:
        ...
        if loop is not self._loop:
            try:
                ...
                pass
            return
        self._submit_coro_on_gateway_loop(coro)

    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        """Schedule approval resolution and build the synchronous callback response."""
        approval_id = _coerce_feishu_approval_id(action_value.get("approval_id"))
        if approval_id is None:
            logger.debug("[Feishu] Card action missing approval_id, ignoring")
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(str(action_value.get("hermes_action") or ""), "deny")
    ...
    def _on_lark_ws_executor_done(self, fut: asyncio.Future) -> None:
        """Worker thread finished — Lark ``start()`` returned or raised.

        Without this hook the gateway can keep running while Feishu is dead.
        """
        if self._ws_future is not fut:
            return
        if fut.cancelled():
            return
        # Planned ``disconnect()`` sets ``_running=False`` before awaiting the future.
        ...
        async def _react() -> None:
            ...
            if exc is None:
                msg = (
                    "Lark websocket worker thread ended (SDK start() returned); "
                    "connection will be re-established by the gateway."
                )
            else:
                msg = f"Lark websocket worker thread ended: {exc!r}"
            logger.error("[Feishu] %s", msg)
            self._set_fatal_error("feishu_ws_worker_exit", msg, retryable=True)
            await self._notify_fatal_error()

        def _kick() -> None:
            try:
                main_loop.create_task(_react(), name="feishu-ws-worker-exit")
            except RuntimeError:
                pass
    ...
    async def _connect_websocket(self) -> None:
        ...
        def _make_ws_client() -> Any:
            return _get_feishu_ws_client_cls()(
                app_id=self._app_id,
        ...
        def _thread_main() -> None:
            try:
                _run_official_feishu_ws_client(adapter_ref, _make_ws_client)
        ...
        self._ws_feishu_thread = threading.Thread(
            target=_thread_main,
            name="hermes-feishu-lark-ws",
            daemon=True,
        )
        self._ws_feishu_thread.start()
        self._ws_future = fut
        self._ws_future.add_done_callback(self._on_lark_ws_executor_done)
